...
const CONTEXT_MENU_ITEMS: ContextMenuItem[] = [
  ...
  { label: "重命名", action: "rename", divider: true },
  { label: "删除", action: "delete" },
  { label: "在资源管理器中打开", action: "reveal_in_explorer", icon: ExternalLink, divider: true },
  { label: "复制路径", action: "copy_path", icon: Copy },
];
...
function TreeNodeRow({
  ...
  onToggleFolder, onSelectFile, onContextAction, onNavigate,
}: TreeNodeRowProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    switch (e.key) {
      case "Enter": e.preventDefault(); handleClick(); break;
      ...
      case "ArrowLeft":
        ...
        break;
      case "F2": e.preventDefault(); onContextAction("rename", node); break;
    }
  };
    ...
    <>
      ...
      {contextMenu && (
        <>
          ...
          />
          <div
            className="fixed z-50 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md shadow-lg py-1 min-w-[180px]"
            style={{ left: contextMenu.x, top: contextMenu.y }}
          >
            {CONTEXT_MENU_ITEMS.map((item) => (
              <div key={item.action}>
                {item.divider && <hr className="my-1 border-slate-100 dark:border-slate-700" />}
                <button
                  type="button"
                  className="w-full text-left px-3 py-1.5 text-xs text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors flex items-center gap-2"
                  onClick={() => { closeContextMenu(); onContextAction(item.action, node); }}
                >
                  {item.icon && <item.icon size={13} className="text-slate-400 dark:text-slate-500" />}
                  {item.label}
                </button>
              </div>
            ))}
...
export default function FileTree({
  ...
  onToggleFolder, onSelectFile, onContextAction,
  level = 0, visibleNodes: propVisibleNodes,
}: FileTreeProps) {
  ...
  // Focus the DOM row when selection changes
  useEffect(() => {
  ...
  const handleNavigate = useCallback(
    (direction: "up" | "down" | "right" | "left") => {
      ...
      if (direction === "down" && idx < visibleNodes.length - 1) {
        ...
        const dir = !!(node.is_directory || node.is_dir);
        if (dir && expandedFolders.has(node.relative_path) && node.children?.length) {
    ...
    [visibleNodes, selectedFile, expandedFolders, onSelectFile],
  );
